        ...
        NavigationView{
            VStack{
                ...
                Text("Notification Preferences ")
                    .font(.system(size: 22, weight:.bold)).frame(width:360, alignment: .leading).padding(.top,0)
                ...
                VStack{
                    ZStack{
                        ...
                        Text("Start Hour").frame(width:330, alignment:.leading).padding(.top)
                        
                        DatePicker("", selection: $StartHour , displayedComponents: .hourAndMinute).frame(width: 320).padding(.top,20)

                    }
                    Divider()
                    ZStack{
                        Rectangle().frame(height:44).foregroundStyle(Color.myFieldsGray)
                        Text("End Hour").frame(width:330, alignment:.leading).padding(.bottom)
                        DatePicker("", selection: $EndHour , displayedComponents: .hourAndMinute).frame(width: 320).padding(.bottom,20)
                    }
                    ...
                    HStack(spacing: 12) {
                        ForEach(Minintervals.indices, id: \.self) { index in
                            ...
                            Button(action: {
                                ...
                                VStack {
                                    Text("\(Minintervals[index])").foregroundColor(isPressed[index] ? Color.white : Color.cyan)
                                    Text("Mins").foregroundColor(isPressed[index] ? Color.white : Color.black)
                    ...
                    HStack(spacing: 12) {
                        ForEach(Hourintervals.indices, id: \.self) { index in
                            ...
                            Button(action: {
                                ...
                                VStack {
                                    Text("\(Hourintervals[index])").foregroundColor(isPressed2[index] ? Color.white : Color.cyan)
                                    Text("Hours").foregroundColor(isPressed2[index] ? Color.white : Color.black)
                                }.frame(width:77, height: 70)
                ...
                Spacer()
                HStack{
                    NavigationLink(destination:LitersCounting()){
                        Text("Start")
                    }
                }.frame(width:355,height:50).background(Color.cyan).foregroundStyle(Color.white).cornerRadius(16).padding(.top,40)
